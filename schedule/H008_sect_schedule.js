...
module.exports.scheduleInsert = () => {
    schedule.scheduleJob(process.env.H008_TIME_SECT, async function() {
        let rtnResult = {};
        ...
        try {
            const result = await db.sequelize.transaction(async (t) => {
                ...
                let rslt = await db[tableName.toUpperCase()].findAll({where: {gubun:'MORE_P', state:'200', sectValue: 'Y'}})
                    .then(async users => {
                        if (users.length) {
                            winston.info("******************* H008 Request(SECT) is found!!! *************************");
                            for (user of users) {
                                ...
                                httpcall.Call('post', process.env.ANOMAL_ADDRESS, value, async function (err, res) {
                                    if(res) {
                                        res.body.result.contents = JSON.stringify(value.body); res.header.sectValue = 'Y';

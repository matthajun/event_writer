        ...
        try {
            ...
            for (tableName of tableNames) {
                const result = await db.sequelize.transaction(async (t) => {
                    ...
                    let rslt = await db[tableName.toUpperCase()].findAll({where: {trans_tag: 'C'}}).then(async users => {
                        if (users.length) {
                            let childTable = [];
                            for (user of users) {
                            ...
                            if(childTable.length > 40){
                                ...
                                for(let daughtTable of motherTable){
                                    ...
                                    await makereq.highrankPush(tableInfo);
                                }
                            }
                            ...
                            winston.info("******************* "+tableName+" Transmit End *************************");
                        }
                    });
                });
            }
